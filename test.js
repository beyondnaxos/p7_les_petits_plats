...
function filter(recipes, string){
	let filtred = []
	for(let i = 0; i < recipes.length ; i++){
		if(
		filterIngredients(recipes[i].ingredients, string)||
		recipes[i].name.toLowerCase().includes(string.toLowerCase()) ||
		recipes[i].description.toLowerCase().includes(string.toLowerCase())
		){
			filtred.push(recipes[i])
		}
	}
	return filtred
}

function filterIngredients(ingredients, string){
	for(let i = 0 ; i< ingredients.length; i++){
		if(ingredients[i].ingredient.toLowerCase().includes(string.toLowerCase())){
			return true
		}
	}
	return false
}
filtredRecipes = filter(recipes, str);
console.log('v3', filtredRecipes.length)